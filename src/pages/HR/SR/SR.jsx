              ...
              <table className="styled-table" ref={tableRef}>
                ...
                <tbody>
                    ...
                    .map((id) => {
                      return (
                              <tr>
                                <td style={{textAlign: "center"}}>{staff[id].staffno}</td>
                                <td style={{textAlign: "center"}}>{staff[id].name}</td>
                                <td style={{textAlign: "center"}}>{staff[id].position}</td>
                                <td style={{textAlign: "center"}}>{staff[id].month}</td>
                                <td style={{textAlign: "center"}}>${staff[id].salary}</td>
                                <td style={{textAlign: "center"}}>${staff[id].commission}</td>
                                <td style={{textAlign: "center"}}>${staff[id].salary > 30000 ? 1500 :(staff[id].salary)*0.05}</td>
                                <td style={{textAlign: "center"}}>${staff[id].salary - (staff[id].salary > 30000 ? 1500 :(staff[id].salary)*0.05)}</td>
                                <td style={{textAlign: "center"}}>
                                  <NavLink to={`update/${id}`}  className='btn-edit' >Edit</NavLink>
                                  <button className='btn-delete' onClick={() => {if(window.confirm("Are you sure that you wanted to delete that Salary and MPF report?")){remove(ref(db, 'salaryreport/' + id)); window.location.reload(true);}}}>Delete</button>
                                  <NavLink to={`view/${id}`} className='btn-view' >Receipt</NavLink>
                                </td>
                              </tr>
                      );
                    })}
