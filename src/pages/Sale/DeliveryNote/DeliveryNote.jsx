          ...
          <div className="text-end" >
            ...
            <button className="btn-create" onClick={() => openInNewTab(resume)}>
              ...
              <table className="styled-table" ref={tableRef}>
                ...
                <tbody>
                    {Object.keys(DeliveryNote).filter(
                      (id) => {
                        return search.toLowerCase() === ''
                          ? DeliveryNote[id]
                          : DeliveryNote[id].no.toLowerCase().includes(search);
                      }
                    ).sort((a,b) => DeliveryNote[a].no > DeliveryNote[b].no ? 1 : -1)
                    .map((id) => {
                      return (
                              <tr>
  
                                <td style={{textAlign: "center"}}>{DeliveryNote[id].no}</td>
                                <td style={{textAlign: "center"}}>{DeliveryNote[id].to}</td>
                                <td style={{textAlign: "center"}}>{DeliveryNote[id].date}</td>
                                <td style={{textAlign: "center"}}>{DeliveryNote[id].item}</td>
                                <td style={{textAlign: "center"}}>{DeliveryNote[id].quantity}</td>
                                <td style={{textAlign: "center"}}>${DeliveryNote[id].price}</td>
                                <td style={{textAlign: "center"}}>${DeliveryNote[id].total}</td>
                                <td style={{textAlign: "center"}}>
                                  <NavLink to={`update/${id}`}  className='btn-edit' >Edit</NavLink>
                                  <button className='btn-delete' onClick={() => {if(window.confirm("Are you sure that you wanted to delete that delivery note?")){remove(ref(db, 'DeliveryNote/' + id)); window.location.reload(true);}}}>Delete</button>
